...
int connect_to_network(wifi_ap_record_t* ap_info, char* pass) {   
    ...
    if(bits & WIFI_CONNECT_SUCCESS) {
        ...
        sprintf(ipAddress, IPSTR, IP2STR(&ip_info.ip));
        ESP_LOGI("WIFI_HELPER", "IP Address: %s", ipAddress);
...
static void handlePackets(const int sock) {
    int len;
    ...
    do {
        ...
        if(len < 0) {
            ...
            memcpy(&msgHeader, rx_buffer, sizeof(MessageHeader_T));

            switch(msgHeader.message_type) {
                case CMD_LED:
                    memcpy(&cmdHeader, rx_buffer + sizeof(MessageHeader_T), sizeof(CommandHeader_T));
                    switch(cmdHeader.command_type) {
                        case SET_FRAME:
                            ESP_LOGI("WIFI_HELPER", "Message Recieved: SET_FRAME");
                            if(xSemaphoreTake(DATA_ARR_MUTEX, portMAX_DELAY) == pdTRUE) {
                                unpack_cmd_set_frame((uint8_t*) rx_buffer, len, DATA_ARR, sizeof(DATA_ARR));
                        ...
                        case SET_ANIMATION:
                            ESP_LOGI("WIFI_HELPER", "Message Recieved: SET_ANIMATION");
                            ...
                            int retval = unpack_set_animation_frame(
                                (uint8_t*) rx_buffer, 2048, animationName, sizeof(animationName)
                            );

                            ESP_LOGI("WIFI_HELPER", "Animation unpack retval: %d", retval);
                            ESP_LOGI("WIFI_HELPER", "Animation Name: %s", animationName);
                ...
                case ANIMATION_CTRL:
                    ESP_LOGI("WIFI_HELPER", "Message Recieved: ANIMATION_CTRL");
                    ANIMATION_CTRL_T enabled;
                    unpack_animation_ctrl_frame((uint8_t*) rx_buffer, len, &enabled);
                    animationEnabled = enabled == PLAY;
                    break;

                case CONNECT_REQUEST:
                    ESP_LOGI("WIFI_HELPER", "Message Recieved: CONNECT_REQUEST");
                    total_bytes_to_send = pack_client_connect_response_handshake_frame(
                        (uint8_t*) tx_buffer, sizeof(tx_buffer), LED_HEIGHT, LED_WIDTH, 
                        DEVICE_ID,  strlen(DEVICE_ID)
                    );

                    remainingSize = total_bytes_to_send;

                    printf("DATA: ");
                    for(int i = 0; i < total_bytes_to_send; i++) printf("%02x ", tx_buffer[i]);
                    printf("\n");

                    ESP_LOGI("WIFI_HELPER", "Connect Response Return Value: %d", total_bytes_to_send);
                    break;
            ...
            while(remainingSize > 0) {
                ESP_LOGI("WIFI_HELPER", "Total bytes remaining: %d", remainingSize);
                int written = send(
                    sock, 
                    tx_buffer + (total_bytes_to_send - remainingSize), 
                    remainingSize, 
                    0
                );
...
void tcp_server_init(int port){
    xTaskCreate(tcp_server_task, "tcp_server", 8192, (void*)port, 5, NULL);
}
